...
:::warning
The files placed under theme folder will be applied to local theme automatically, no need to edit `config.js`.
:::
...
- [Markdown Slot](https://v1.vuepress.vuejs.org/guide/markdown-slot.md)

## Directory Structure

One `Layout.vue` might not be enough, and you might also want to define more layout components in the theme for using on different pages. You may also want to customize the [palette](https://v1.vuepress.vuejs.org/config/README.md#palette-styl), and even apply some plugins.
...
::: tip
Each layout component may render distinct pages. To apply some global UI (for example global header), consider using [globalLayout](https://v1.vuepress.vuejs.org/theme/option-api.md#globallayout)。
:::
...
::: tip
`lastUpdated` is the UNIX timestamp of this file’s last git commit, for more details, check out [Last Updated](https://v1.vuepress.vuejs.org/theme/default-theme-config.md#last-updated).
:::
